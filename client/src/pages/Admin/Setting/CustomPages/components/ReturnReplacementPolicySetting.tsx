...
const ReturnReplacementPolicySetting = () => {
    ...
    const savePageSetting = async (values: any) => {
        try {
            ...
            const response: any = await apis.SavePageSetting({ ...values, page: 'returnReplacementPolicy' });
            if (response.success) {
                dispatch(setPages(response.pages));
                messageAPI.open({
                    type: "success",
                    content: "Return and Replacement Policy page setting saved successfully",
                });
            }
    ...
    React.useEffect(() => {
        if (pages && pages.returnReplacementPolicy && form) {
            if (pages.returnReplacementPolicy.text && pages.returnReplacementPolicy.text.blocks) {
                setEditorState(EditorState.createWithContent(convertFromRaw(pages.returnReplacementPolicy.text)))
            }
            if (pages.returnReplacementPolicy.image) {
                setFileList([
                    {
                        uid: "-1",
                        name: "returnReplacementPolicy",
                        status: "done",
                        url: `${process.env.REACT_APP_SERVER_URL}/${pages.returnReplacementPolicy.image}`,
                    },
                ]);
            }
        }
...
export default ReturnReplacementPolicySetting;