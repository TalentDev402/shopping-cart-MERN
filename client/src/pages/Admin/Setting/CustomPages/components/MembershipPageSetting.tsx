...
const MembershipPageSetting = () => {
    ...
    const saveMembershipPageSetting = async (values: any) => {
        try {
            ...
            const response: any = await apis.SavePageSetting({ ...values, page: 'membership' });
            if (response.success) {
                dispatch(setPages(response.pages));
                messageAPI.open({
                    type: "success",
                    content: "Membership page setting saved successfully",
                });
            }
    ...
    React.useEffect(() => {
        if (pages && pages.membership && form) {
            if (pages.membership.text && pages.membership.text.blocks) {
                setEditorState(EditorState.createWithContent(convertFromRaw(pages.membership.text)))
            }
            if (pages.membership.image) {
                setFileList([
                    {
                        uid: "-1",
                        name: "membership",
                        status: "done",
                        url: `${process.env.REACT_APP_SERVER_URL}/${pages.membership.image}`,
                    },
                ]);
            }
        }
    ...
    return (
        <Card style={{ padding: 0 }}>
            ...
            <Form
                ...
                onFinish={saveMembershipPageSetting}
                autoComplete="off"
            >
                <Form.Item
...
export default MembershipPageSetting;