...
const ShippingAndPaymentTermSetting = () => {
    ...
    const savePageSetting = async (values: any) => {
        try {
            ...
            const response: any = await apis.SavePageSetting({ ...values, page: 'shippingPaymentTerms' });
            if (response.success) {
                dispatch(setPages(response.pages));
                messageAPI.open({
                    type: "success",
                    content: "shipping And Payment Terms page setting saved successfully",
                });
            }
    ...
    React.useEffect(() => {
        if (pages && pages.shippingPaymentTerms && form) {
            if (pages.shippingPaymentTerms.text && pages.shippingPaymentTerms.text.blocks) {
                setEditorState(EditorState.createWithContent(convertFromRaw(pages.shippingPaymentTerms.text)))
            }
            if (pages.shippingPaymentTerms.image) {
                setFileList([
                    {
                        uid: "-1",
                        name: "shippingPaymentTerms",
                        status: "done",
                        url: `${process.env.REACT_APP_SERVER_URL}/${pages.shippingPaymentTerms.image}`,
                    },
                ]);
            }
        }
...
export default ShippingAndPaymentTermSetting;