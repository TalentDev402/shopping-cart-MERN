...
const PrivacyPolicySetting = () => {
    ...
    const savePageSetting = async (values: any) => {
        try {
            ...
            const response: any = await apis.SavePageSetting({ ...values, page: 'privacyPolicy' });
            if (response.success) {
                dispatch(setPages(response.pages));
                messageAPI.open({
                    type: "success",
                    content: "Privacy Policy page setting saved successfully",
                });
            }
    ...
    React.useEffect(() => {
        if (pages && pages.privacyPolicy && form) {
            if (pages.privacyPolicy.text && pages.privacyPolicy.text.blocks) {
                setEditorState(EditorState.createWithContent(convertFromRaw(pages.privacyPolicy.text)))
            }
            if (pages.privacyPolicy.image) {
                setFileList([
                    {
                        uid: "-1",
                        name: "deliveryMethod",
                        status: "done",
                        url: `${process.env.REACT_APP_SERVER_URL}/${pages.privacyPolicy.image}`,
                    },
                ]);
            }
        }
