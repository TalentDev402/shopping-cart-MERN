...
const DeliveryMethodSetting = () => {
    ...
    const savePageSetting = async (values: any) => {
        try {
            ...
            const response: any = await apis.SavePageSetting({ ...values, page: 'deliveryMethod' });
            if (response.success) {
                dispatch(setPages(response.pages));
                messageAPI.open({
                    type: "success",
                    content: "Delivery Methods page setting saved successfully",
                });
            }
    ...
    React.useEffect(() => {
        if (pages && pages.deliveryMethod && form) {
            if (pages.deliveryMethod.text && pages.deliveryMethod.text.blocks) {
                setEditorState(EditorState.createWithContent(convertFromRaw(pages.deliveryMethod.text)))
            }
            if (pages.deliveryMethod.image) {
                setFileList([
                    {
                        uid: "-1",
                        name: "deliveryMethod",
                        status: "done",
                        url: `${process.env.REACT_APP_SERVER_URL}/${pages.deliveryMethod.image}`,
                    },
                ]);
            }
        }
...
export default DeliveryMethodSetting;