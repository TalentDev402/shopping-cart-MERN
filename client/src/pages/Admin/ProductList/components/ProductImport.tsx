...
const ProductImport: React.FC<ProductModalProps> = ({
  open,
  setOpen,
  refreshList
}) => {
  ...
  const saveProduct: FormProps["onFinish"] = async (values) => {
    try {
      ...
      let response: any = await apis.CreateProduct(values);
      if (response.success) {
        refreshList();
      }
      messageAPI.open({
      ...
      >
        ...
        >
          <Flex justify="flex-end" gap={10}>
            <Button type="primary" onClick={importProduct} loading={confirmLoading}>
        ...
        <Form.Item
          ...
          name="description"
        >
          <Input.TextArea style={{ height: 150 }} disabled />
        </Form.Item>
        ...
        <Form.Item style={{ marginBottom: 8 }} label="Quantity" name="quantity">
          <InputNumber style={{ width: "100%" }} min={0} step="1" disabled />
        </Form.Item>
